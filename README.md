...
目前分为三个文件：`OJindex.html`与`vertwo.html`以及`verthree.html`。第一个文件是前作，比较简洁；第二个是硬核版，更加复杂；第三个文件是 V 3.0，支持小游戏、自主出题。
...
## 请注意，这个网站为民间自发组织维护，非中国领事服务网（<https://cs.mfa.gov.cn/>）与中国外交部（<https://www.fmprc.gov.cn/>）官网及官方指定网站，并与其无关且无隶属关系，且不保证更新速率及网站稳定性，如有需要请及时联系中国大使馆（12308）及当地警方。

## 当前缺乏信息维护与更新，请关注官方最新消息！

分为两个文件：

- `ChineseEmbassyV4.html`
- `ChineseEmbassyV5.html`

其中`ChineseEmbassyV4.html`因为**严重的管理员权限泄露风险而被停止支持，后台数据也已全部删除**，现在运营的是`ChineseEmbassyV5.html`。
...
- **取消**了三步验证码
- **修复**了管理员入口登录按钮点击无响应的问题
- **取消**了管理员入口
- **新增**了官方链接集锦
- **修复**了站点信息显示全面崩盘的问题
- **移除**了未经验证的官方链接集锦中的链接，**替换**为了中国领事服务网的注意安全提醒
- **修改**了原`安全提醒专栏`提示文字为`安全提醒、暂勿前往专栏`
